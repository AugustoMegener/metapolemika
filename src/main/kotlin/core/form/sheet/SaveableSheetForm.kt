...
@Serializable(SaveableSheetForm.Serializer::class)
@SerialName(SHEET)
class SaveableSheetForm(val form: SheetForm<*>) {

    object Serializer : KSerializer<SaveableSheetForm> {
        override val descriptor = buildClassSerialDescriptor("sheet")

        @OptIn(ExperimentalSerializationApi::class)
        override fun deserialize(decoder: Decoder): SaveableSheetForm =
            with(decoder.beginStructure(descriptor)) {
                ...
                SaveableSheetForm(sheet)
            }


        @OptIn(ExperimentalSerializationApi::class)
        override fun serialize(encoder: Encoder, value: SaveableSheetForm) {
            with(encoder.beginStructure(descriptor)) {
                var i = 0

                encodeStringElement(descriptor, i++, value.form.id)
                encodeSerializableElement(descriptor, i++, serializer<BaseSheetForm>(), value.form.sheet)

                value.form.fields.forEach { encodeNullableSerializableElement(descriptor, i++, serializer<String>(), it.input) }
            }
        }
